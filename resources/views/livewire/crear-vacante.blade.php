<form class="md:w-1/2 space-y-5" method="POST" enctype="multipart/form-data" wire:submit.prevent='crearVacante'>
    ...
    <div>
        ...
        <x-input 
            ...
            wire:model="empresa" 
            :value="old('empresa')" 
            placeholder="Empresa: ej. Google, Facebook"
        />
        @error('empresa')
            <livewire:mostrar-alerta :message="$message" />
        @enderror
    ...
    <div>
        <x-label for="ultimo_dia" :value="__('Último día para postular')" />
        <x-input 
            id="ultimo_dia" 
            class="block mt-1 w-full" 
            type="date" 
    ...
    <div>
        ...
        <textarea
            ...
            class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 
            focus:ring focus:ring-indigo-200 focus:ring-opacity-50 w-full h-48"
        ></textarea>
        @error('descripcion')
            <livewire:mostrar-alerta :message="$message" />
        @enderror
    ...
    <div>
        ...
        <div class="my-5 w-full text-md text-gray-800 text-center">
            @if ($imagen)
                Imagen
